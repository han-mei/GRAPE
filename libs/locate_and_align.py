...
for funcInfoFile in os.listdir(path):
    if funcInfoFile[-4:] != '.txt' or funcInfoFile[:4] == 'diff':     continue
    f = open(path + funcInfoFile)
    results = f.read()
    f.close()
    resultList = json.loads(results)
    for item in resultList:
        if '_1' in item.keys() and '_2' in item.keys() and '_3' in item.keys():
            filename = '/' + funcInfoFile[4:-4].replace('_', '/')+'.java'
            funcInfo.append((filename, item['_2'], item['_3']))

if os.path.exists(path + 'diff.txt'):
